...
gen_proto_dotnet() {
  ...
  mkdir -p ./src/protos/
  cp -r "$base_dir"/proto/nexurarc/protos/
  cd "$base_dir" || return
}
...
gen_proto_elixir() {
  ...
  cd "$base_dir"/src/"$1" || return
  mkdir -p proto
  cp "$base_dir"/proto/nexura.proto ./proto/nexura.proto
  rebar3 grpc_regen
  cd "$base_dir" || return
}
...
gen_proto_go() {
  ...
  cd "$base_dir"/src/"$1" || return
  protoc -I ../../pb ./../../proto/nexura.proto --go_out=./ --go-grpc_out=./
  cd "$base_dir" || return
}
...
gen_proto_js() {
  ...
  cd "$base_dir"/src/"$1" || return
  cp "$base_dir"/proto/nexura.proto .
  cd "$base_dir" || return
}
...
gen_proto_python() {
  ...
  cd "$base_dir"/src/"$1" || return
  python3 -m grpc_tools.protoc -I=../../pb --python_out=./ --grpc_python_out=./ ./../../proto/nexura.proto
  cd "$base_dir" || return
}
...
gen_proto_ts() {
  ...
  cp -r "$base_dir"/proto .
  mkdir -p ./protos
  protoc -I ./proto  --plugin=./node_modules/.bin/protoc-gen-ts_proto --ts_proto_opt=esModuleInterop=true --ts_proto_out=./protos --ts_proto_opt=outputServices=grpc-js nexura.proto
  cd "$base_dir" || return
}
