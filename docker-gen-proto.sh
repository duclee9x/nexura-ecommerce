...
gen_proto_ts() {
  ...
  docker build -f "src/$1/genproto/Dockerfile" -t "$1-genproto" .
  docker run --rm -e SERVICE=$1 "$1-genproto" /bin/sh -c '
    protoc --proto_path=./ \
    --plugin=./node_modules/.bin/protoc-gen-ts_proto \
    --ts_proto_opt=esModuleInterop=true \
    --ts_proto_out="/build/protos" \
    --ts_proto_opt=outputServices=grpc-js \
    ./nexura.proto && cat /build/protos/nexura.ts' > "$(pwd)/src/$1/genproto/nexura.ts"
    
}

# protoc --proto_path=./ --plugin=./node_modules/.bin/protoc-gen-ts_proto --ts_proto_opt=esModuleInterop=true --ts_proto_out="./protos" --ts_proto_opt=outputServices=grpc-js ./nexura.proto'
