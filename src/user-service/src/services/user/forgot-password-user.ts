...
export const forgotPassword = async (
  ...
  try {
    // Find user
    logger.info('Forgot password request', { email: call.request })
    ...
    const recentOTP = await withTracing(tracer, 'Check Recent OTP Attempts', async (span) => {
      ...
      // Create new OTP
      await withTracing(tracer, 'Create OTP Record', async () => {
...
const sendOTPToEmail = async (email: string, otp: string, resetToken: string) => {
  await withTracing(tracer, 'sendOTPToEmail', async (span) => {
    api.context.with(api.trace.setSpan(api.context.active(), span), async () => {
      ...
      const result = await client.binding.send(BINDING_NAME, BINDING_OPERATION, `Your OTP is ${otp} and your reset token is ${resetToken}`, {
        emailTo: email,
        subject: "Nexura - Reset Password",
      }
      )
      console.log(result)
    });
