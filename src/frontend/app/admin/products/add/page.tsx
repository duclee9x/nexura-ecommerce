...
const productCategories = ["sneakers", "clothing", "accessory","shoes","watches","backpack", "bag","other"]
const productTypes = ["everyday", "travel", "sport", "business","none"]
...
type ComponentsStatusActionType = 
  | { type: "setActiveTab"; payload: string }
  | { type: "setIsUnsaved"; payload: boolean }
  | { type: "setIsSaving"; payload: boolean }
  | { type: "setNewFeature"; payload: string }
  | { type: "setDraggedImageIndex"; payload: number | null }

const componentsStatusReducer = (state: ComponentsStatusState, action: ComponentsStatusActionType): ComponentsStatusState => {
  switch (action.type) {
    case "setActiveTab":
      return { ...state, activeTab: action.payload }
    case "setIsUnsaved":
      return { ...state, isUnsaved: action.payload }
    case "setIsSaving":
      return { ...state, isSaving: action.payload }
    case "setNewFeature":
      return { ...state, newFeature: action.payload }
    case "setDraggedImageIndex":
      return { ...state, draggedImageIndex: action.payload }
    default:
      return state
  }
}
...
export default function ProductAddPage() {
  ...
  // Product state
  const [product, setProduct] = useReducer((state: Product, action: Partial<Product>) => {
    return { ...state, ...action }
  }, {
    name: "",
    spu: "",
  ...
  // Form state
  const [componentsStatus, componentsStatusDispatch] = useReducer(componentsStatusReducer, {
    activeTab: "basic",
    isUnsaved: false,
    isSaving: false,
    newFeature: "",
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name.includes(".")) {
      ...
      const parentKey = parent as keyof Product
      const parentValue = product[parentKey]
      ...
      setProduct({ [name]: value } as Partial<Product>)
    }
  ...
  const handleToggleChange = (name: string, checked: boolean) => {
    setProduct({ [name]: checked } as Partial<Product>)
    setIsUnsaved(true)
  }
  ...
  const handleColorChange = (color: string) => {
    ...
    if (colors.includes(color)) {
      setProduct({ colors: colors.filter((c) => c !== color) })
    } else {
      setProduct({ colors: [...colors, color] })
    }
    setIsUnsaved(true)
  }
  ...
  const handleAddFeature = () => {
    ...
    setProduct({ features: [...product.features, newFeature.trim()] })
    setNewFeature("")
    setIsUnsaved(true)
  }
  ...
  const handleRemoveFeature = (index: number) => {
    setProduct({
      features: product.features.filter((_, i: number) => i !== index),
    })
    setIsUnsaved(true)
  }
  ...
  const handleRelatedProductToggle = (id: number) => {
    ...
    if (relatedProductIds.includes(id)) {
      setProduct({
        relatedProductIds: relatedProductIds.filter((pid: number) => pid !== id),
      })
    } else {
      setProduct({
  ...
  const handleRemoveImage = (index: number) => {
    setProduct({
      images: product.images.filter((_, i: number) => i !== index),
    })
    setIsUnsaved(true)
  }
        ...
        <main className="flex-1 p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center">
              ...
              <div>
                <h1 className="text-3xl font-bold dark:text-white">Add Product</h1>
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input id="name" name="name" value={product.name} placeholder="Adidas Superstar" onChange={handleInputChange} />
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="spu">SPU</Label>
                      <Input id="spu" name="spu" value={product.spu} placeholder="ADI-SUP-001" onChange={handleInputChange} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input id="materials" placeholder="e.g., Cotton, Polyester, Leather" name="materials" value={product.materials} onChange={handleInputChange} />
                    </div>
